...
export class AuthService {
    ...
    async register({ username, password }: RegisterDto) {
        const usuarioFound = await this.usuarioService.findOneByUsername(username)
    ...
    async login({ username, password }: LoginDto) {
        ...
        const payload = { username: usuarioFound.username, role: usuarioFound.role };
    ...
    async perfil({ username, role }: { username: string, role: string }) {
        // if (role === 'admin') {
